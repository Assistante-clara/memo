...
if (data.memo == "on") {      
  if(query==""){ClaraIASpeech('rien à ajouter') ;return }
    if (jsonStr.indexOf(query) > -1  ){JarvisIASpeech (query + ", il est déja présent dans la liste|il est déjà inscrit dans la vie|il a déjà était dis");}
      else {ClaraIASpeech(query+" ajouté à la liste ")
...
if (data.memo=="vide"){
     ...
     ClaraIASpeech("suppression de tous les mémo|j'espère que tu as tout fait|voilà j'ai tout éffacer|c'est bon");
     return
}//fin if

if (data.memo == "pushliste"){
 ClaraIA.reco= "envoie une notification à fred "+jsonStr
 ClaraIARun(['sendsms','data.destinataire="fred",data.numero="XXXXXXXXXX",data.pharsesms="envoie une notification à fred"'])
return 
}//fin if
return
}
